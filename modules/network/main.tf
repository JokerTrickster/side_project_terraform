...
module "nat_instance"{
  ...
  vpc_cidr = module.vpc.cidr_block
  key_name = var.key_name
  dev_common_nat_instance_network_interface_id = module.dev_common_nat_instance.nat_instance_network_interface_id
...
resource "aws_route" "private_nat_instance_route" {
  count                  = length(var.private_subnet_cidrs)
  route_table_id         = element(module.private_subnet.route_table_ids, count.index)
  network_interface_id            = module.dev_common_nat_instance.nat_instance_network_interface_id
  destination_cidr_block = var.destination_cidr_block
}
resource "null_resource" "dummy_dependency" {
